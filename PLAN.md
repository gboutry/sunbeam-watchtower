...
- canonical artifact discovery: `internal/core/service/artifactdiscovery` is the single HEAD-tree-based discoverer for charm, snap, and rock artifacts (via a package-local `TreeReader` seam, with a `gitcache` adapter wired through `internal/app/artifactdiscovery_bootstrap.go`). It supports root-layout and mono-repo layouts (`charms/*`, `charms/*/*`, `snaps/*/snap`, `rocks/*`), extracts charm resources, and produces deterministic sorted output. Unit tests cover single-charm root, charm and nested-charm mono-repos, snap root and mono-repo, rock mono-repo, missing manifests, and malformed YAML.
- team sync mono-repo fan-out: `TeamServerWorkflow.Sync` routes each configured project through `app.GitCache().EnsureRepo` and the canonical `artifactdiscovery` service, then emits one `dto.SyncTarget` per discovered artifact instead of one per project. This fixes `HTTP 404: Name sunbeam-charms not found in the charm namespace` for mono-repos — `sunbeam-charms` now fans out to ~50 `ArtifactSyncResult` rows, one per charm under `charms/*/charmcraft.yaml`, while single-artifact projects like `openstack` still return exactly one row with the snap name read from `snap/snapcraft.yaml`. A new optional `proj.team.skip_artifacts` list filters individual artifacts out of the fan-out (mirrors the existing `proj.release.skip_artifacts` pattern and is a no-op when the `team` block is absent). Discovery failures (bad clone URL, cache miss, parse error, empty manifest set) are surfaced as per-project warnings on the result rather than aborting the whole sync, so one unreachable project does not block collaborator invites for its siblings. Authorization surface: the `team.sync` ActionID is unchanged — same mutability (mutating on apply, dry-run read-only), same runtime requirement, same MCP export policy — but the fan-out widens reach so a single `team sync` invocation may now issue one `InviteCollaborator` call per charm in a mono-repo where previously it would fail before any store call was made. The dead `frontend/team_discovery.go` primitive has been removed now that the canonical service is the only discovery path for team sync.
...
- finish consolidating artifact discovery onto the canonical `internal/core/service/artifactdiscovery` service: release bootstrap (`discoverSnapName` / `discoverCharmPublications` in `internal/app/release_helpers.go`) and `build.Strategy.DiscoverRecipes` still carry their own parallel discoverers. Migrate both onto the service (worktree walker in `build` can stay; only the leaf manifest parser needs to be shared) and delete the legacy helpers once all callers have moved over.
