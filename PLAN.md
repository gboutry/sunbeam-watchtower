...
- **Packages / Excuses** — provider-owned feed URLs with HEAD-based freshness checks, structured autopkgtest parsing, `--set`/`--blocked-by-set` filtering, CLI OSC 8 hyperlinks; package cache sync validates explicit distro/release/backport filters and keeps distro releases separate from configured backport sources
