...
- local build preparation now preserves nested monorepo layouts: `ArtifactStrategy.DiscoverRecipes` returns `[]DiscoveredRecipe{Name, RelPath}` instead of bare names, and `CharmStrategy`/`RockStrategy` walk the `charms/`/`rocks/` subtrees with `filepath.WalkDir` (stopping at the first metadata hit per subtree) so charms like `charms/storage/foo/charmcraft.yaml` report `RelPath = "charms/storage/foo"`. `LocalBuildPreparer.PrepareTrigger` prefers the discovered `RelPath` when populating `PreparedBuildRecipe.BuildPath`, and falls back to the shallow name-based `resolveBuildPath` heuristic only when the caller passed `--artifacts` explicitly without running discovery. `SnapStrategy` still reports single-artifact repos with an empty `RelPath`. Previously the shallow glob `charms/*/charmcraft.yaml` silently missed nested charms and the flat fallback `charms/<name>` sent Launchpad to the wrong directory.
