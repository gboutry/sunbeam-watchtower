...
- **Build** — local-path preparation runs in an isolated git worktree, optionally executes `prepare_command`, pushes to a temp branch on the user's LP repo, and triggers LP recipes; supports `--retry N` for waited builds and artifact downloads, `--wait` with structured timeout details, aggregate artifact download failures, cleanup of temp recipes and branches, monorepo layouts, and snap platform auto-detection from `snapcraft.yaml`
