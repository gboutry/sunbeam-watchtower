...
- **Build** — local-path preparation runs in an isolated git worktree, optionally executes `prepare_command`, pushes to a temp branch on the user's LP repo, and triggers LP recipes; supports `--retry N` for waited builds and artifact downloads, automatic retries for transient Launchpad API read failures, `--wait` with structured timeout details, aggregate artifact download failures, cleanup of temp recipes and branches, monorepo layouts, and snap platform auto-detection from `snapcraft.yaml`
...
- **Bugs** — cache-first sync with incremental overlap, cached Launchpad comments, `since` filtering on task activity, result limiting, group-aware `--merge` output via `bug_groups` config, and bounded explainable search across complete cached bug text; search is strictly offline, rejects incompatible cache schemas, omits sensitive records, saturates repeated comment evidence, uses weighted concept/clause coverage, and shares deterministic lexical/fuzzy/regex behavior across API/CLI/TUI
...
- **Packages / Excuses** — provider-owned feed URLs with HEAD-based freshness checks, structured autopkgtest parsing, `--set`/`--blocked-by-set` filtering, CLI OSC 8 hyperlinks; package diff/show-version default upstream comparison uses the provider's latest available versions when no upstream release is requested; upstream cache sync updates bare branch refs; package cache sync validates explicit distro/release/backport filters and keeps distro releases separate from configured backport sources
- **Git cache** — bare repo caches refresh both remote-tracking refs and local branch refs so `HEAD`-tree consumers such as artifact discovery and release metadata see current manifests; failed fetches are surfaced as sync warnings instead of counted as synced repos
