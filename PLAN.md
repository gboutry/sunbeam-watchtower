...
- **Bugs** — cache-first sync with incremental overlap, cached Launchpad comments, `since` filtering on task activity, result limiting, and group-aware `--merge` output via `bug_groups` config
