...
func (m *mockCache) Update(_ context.Context, name string, _ []port.SourceEntry) error {
	return nil
}

func (m *mockCache) Query(_ context.Context, name string, opts port.QueryOpts) ([]distro.SourcePackage, error) {
	pkgs := m.data[name]
	if pkgs == nil {
		return nil, nil
	}
...
func (m *mockCache) Status() ([]port.CacheStatus, error) {
	var statuses []port.CacheStatus
	for name, pkgs := range m.data {
		statuses = append(statuses, port.CacheStatus{
			Name:        name,
			EntryCount:  len(pkgs),
...
func TestList(t *testing.T) {
	...
	pkgs, err := svc.List(ctx, "ubuntu", port.QueryOpts{
		Suites: []string{"noble"},
	})
	if err != nil {
		t.Fatal(err)
...
func TestReverseDepends(t *testing.T) {
	...
	results, err := svc.ReverseDepends(ctx, "python3-neutron-lib", []ProjectSource{{Name: "ubuntu"}}, port.QueryOpts{})
	if err != nil {
		t.Fatal(err)
	}
	if len(results) != 2 {
...
func TestReverseDependsPythonAlias(t *testing.T) {
	...
	results, err := svc.ReverseDepends(ctx, "python3-oslo.messaging", []ProjectSource{{Name: "ubuntu"}}, port.QueryOpts{})
	if err != nil {
		t.Fatal(err)
	}
	if len(results) != 2 {
