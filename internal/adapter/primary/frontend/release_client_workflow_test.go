...
func TestReleaseClientWorkflowListAndShow(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/releases":
			...
			_ = json.NewEncoder(w).Encode(map[string]any{"releases": []dto.ReleaseListEntry{{Name: "snap-openstack", Track: "2024.1", Risk: dto.ReleaseRiskStable}}})
	...
	list, err := workflow.List(context.Background(), ReleasesListRequest{Tracks: []string{"2024.1"}})
	if err != nil {
