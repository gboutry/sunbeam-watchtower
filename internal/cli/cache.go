	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if wantCacheType(args, cacheTypeGit) {
				...
				result, err := opts.Client.CacheSyncGit(cmd.Context(), appclient.CacheSyncGitOptions{
					Project: project,
				})
				if err != nil {
					return err
			...
			if wantCacheType(args, cacheTypePackagesIndex) {
				...
				if err := opts.Client.PackagesCacheSync(cmd.Context(), appclient.PackagesCacheSyncOptions{
					Distros:   distros,
					Releases:  releases,
					Backports: backports,
				}); err != nil {
